...
class AnalysisMachine(object):
            ...
            for b in b_list:
                ...
                for training_method in self.ALL_TRAIN_METHODS:
                    ...
                    for query_method in self.ALL_QUERY_METHODS:
                        if not query_method in finished_exp[init_mode][b][training_method]:
                            finished_exp[init_mode][b][training_method][query_method] = {}
                        for dataset_rand_seed in self.ALL_DATASET_RAND_SEED:
                            paths_dict = prepare_save_dir(self.dataset_save_path,
                                                          self.data_download_path,
                                                          self.trainer_save_dir,
                            ...
                            if os.path.exists(paths_dict['test_result_path']):
                                ...
                                finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed] = test_result

                                if draw_perclass:
                                    query_result = torch.load(paths_dict['query_result_path'], map_location=torch.device('cpu'))
                                    finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['query_result'] = query_result
                                    dataset_info = torch.load(paths_dict['dataset_info_path'], map_location=torch.device('cpu'))
                                    finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['initial_discovered_classes'] = dataset_info['discovered_classes']
                                    finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['open_classes'] = dataset_info['open_classes']
                                    ...
                                    finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['train_labels'] = train_labels
                                finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['open_results'] = {}
                                ...
                                for o_method in global_setting.OPEN_SET_METHOD_DICT[training_method]:
                                    if os.path.exists(paths_dict['open_result_paths'][o_method]):
                                        ...
                                        finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['open_results'][o_method] = {}
                                        finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['open_results'][o_method]['auroc'] = open_result['roc']['auc_score']
                                        finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['open_results'][o_method]['roc'] = open_result['roc']
                                        finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['open_results'][o_method]['augoscr'] = open_result['goscr']['auc_score']
                                        finished_exp[init_mode][b][training_method][query_method][dataset_rand_seed]['open_results'][o_method]['goscr'] = open_result['goscr']
                                finished += 1
                            else:
                                unfinished += 1
                                ...
                                if b in finished_exp[init_mode]:
                                    if training_method in finished_exp[init_mode][b]:
                                        if query_method in finished_exp[init_mode][b][training_method]:
                                            if len(finished_exp[init_mode][b][training_method][query_method].keys()) >= 1:
                                                # at least one experiment is finished
                                                is_ready = True
                                if is_ready:
                                    all_res = np.array([float(finished_exp[init_mode][b][training_method][query_method][s][item]) for s in finished_exp[init_mode][b][training_method][query_method].keys()])
                                    res = float(all_res.mean())
                                    std = float(all_res.std())
                                    detail_dict[init_mode][training_method][query_method][b] = {'res': res, 'err' : std, 'count' : len(finished_exp[init_mode][b][training_method][query_method].keys())}
                                    y[idx] = res
                                    err[idx] = std
                            ...
                            for query_method in ALL_QUERY_METHODS:
                                ...
                                for idx, b in enumerate(budget_list):
                                    ...
                                    if b in finished_exp[init_mode]:
                                        if training_method in finished_exp[init_mode][b]:
                                            if query_method in finished_exp[init_mode][b][training_method]:
                                                for s in finished_exp[init_mode][b][training_method][query_method].keys():
                                                    if o_method in finished_exp[init_mode][b][training_method][query_method][s]['open_results']:
                                                        is_ready = True
                                                        count += 1
                                    if is_ready:
                                        all_res = np.array([float(finished_exp[init_mode][b][training_method][query_method][s]['open_results'][o_method][item]) for s in finished_exp[init_mode][b][training_method][query_method].keys()])
                                        # res = float(finished_exp[init_mode][b][training_method][query_method]['open_results'][o_method][item])
                                        res = float(all_res.mean())
                                        std = float(all_res.std())
                                        detail_dict[init_mode][training_method][query_method][b] = {'res': res, 'err' : std, 'count' : count}
                                        y[idx] = res
                                if np.any(np.isfinite(y)):
                                    lines += 1
    ...
    def _draw_open_set_plot(self, finished_exp, key, comparsion_dict, total_size, regular_size, fewer_size, budget_ratio, draw_seen_line=True, error_bar=True):
                ...
                for training_method in ALL_TRAIN_METHODS:
                    ...
                    for query_method in ALL_QUERY_METHODS:
                        for b in sorted(finished_exp[init_mode]):
                            if training_method in finished_exp[init_mode][b]:
                                if query_method in finished_exp[init_mode][b][training_method]:
                                    seen_rates = np.array([float(finished_exp[init_mode][b][training_method][query_method][s]['seen']) for s in finished_exp[init_mode][b][training_method][query_method]])
                        ...
                        for o_method in global_setting.OPEN_SET_METHOD_DICT[training_method]:
                            ...
                            for idx, b in enumerate(budget_list):
                                ...
                                if is_ready:
                                    ...
                                    all_res = np.array([float(finished_exp[init_mode][b][training_method][query_method][s]['open_results'][o_method][item]) for s in finished_exp[init_mode][b][training_method][query_method]])
                                    res = float(all_res.mean())
                                    std = float(all_res.std())
        ...
        for item in ['roc', 'goscr']:
            for init_mode in self.ALL_INIT_MODES:
                if init_mode in 'regular':
                    budget_list = regular_b_list
                    init_size = regular_size
                else:
                            ...
                            for idx, b in enumerate(budget_list):
                                ...
                                for o_method in global_setting.OPEN_SET_METHOD_DICT[training_method]:
                                    ...
                                    if b in finished_exp[init_mode]:
                                        if training_method in finished_exp[init_mode][b]:
                                            if query_method in finished_exp[init_mode][b][training_method]:
                                                if seed in finished_exp[init_mode][b][training_method][query_method]:
                                                    if o_method in finished_exp[init_mode][b][training_method][query_method][seed]['open_results']:
                                                        is_ready = True
                                    if is_ready:
                                        res = finished_exp[init_mode][b][training_method][query_method][seed]['open_results'][o_method][item]
                                        x = res['fpr']
                                        if item == 'roc':
                                            y = res['tpr']
                                ...
                                plt.tight_layout()
                                save_dir = os.path.join(path, init_mode, training_method, query_method, f"budget_{b}", f"seed_{seed}")
                                utils.makedirs(save_dir)
                                save_path = os.path.join(save_dir, item+".png")
                                plt.savefig(save_path)
                                plt.close('all')
                        ...
                        for idx, b in enumerate(budget_list):
                            ...
                            if b in finished_exp[init_mode]:
                                if training_method in finished_exp[init_mode][b]:
                                    if query_method in finished_exp[init_mode][b][training_method]:
                                        if seed in finished_exp[init_mode][b][training_method][query_method]:
                                            # if 'softmax' in finished_exp[init_mode][b][training_method][query_method][seed]['open_results']:
                                            is_ready = True
                            if is_ready:
                                test_result = finished_exp[init_mode][b][training_method][query_method][seed]['query_result']
                                all_result = finished_exp[init_mode][b][training_method][query_method][seed]['closed_set_result']
                                initial_discovered_classes = finished_exp[init_mode][b][training_method][query_method][seed]['initial_discovered_classes']
                                open_classes = finished_exp[init_mode][b][training_method][query_method][seed]['open_classes']
                                train_labels = finished_exp[init_mode][b][training_method][query_method][seed]['train_labels']
    ...
    def _get_exp_name(self, init_mode, training_method, query_method, b, dataset_rand_seed, silent=False):
